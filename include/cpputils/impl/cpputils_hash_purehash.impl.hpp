...
struct ItemVoid final : public PureHash::Item<void> {
...
inline int PureHash::getReserveUniqueIdForDataInline(void) const noexcept {
...
inline int PureHash::getReserveUniqueIdForKeyAndHasherInline(void) const noexcept {
...
template <typename TypeData>
typename PureHash::Iterator<TypeData>
PureHash::findNextTheSame( const PureHash::Iterator<TypeData>& CPPUTILS_ARG_NN a_prev ) const noexcept
{
    const ph::ItemVoid* const itemPrevVoid_p = (const ph::ItemVoid*)a_prev;
    const CinternalHashItem_t hsIter = CInternalHashFindNextTheSame(m_clhash_data_p->m_hash,itemPrevVoid_p->hashIter);
    if(hsIter){
        return (PureHash::Iterator<TypeData>)hsIter->data;
    }
    return nullptr;
}
...
{
    Item<TypeData>* const pNewItem = (Item<TypeData>*)((*(m_clhash_data_p->m_hash->allocator))(sizeof(ph::ItemVoid)));
    if (!pNewItem) {
        throw ::std::bad_alloc();
    }
    ph::ItemVoid* const pNewItemVoid = (ph::ItemVoid*)pNewItem;
...
{
    ph::ItemVoid* const pItemVoid = (ph::ItemVoid*)a_iter;
    ph::CKeyBase* const pKeyExt = (ph::CKeyBase*)(pItemVoid->hashIter->key);
