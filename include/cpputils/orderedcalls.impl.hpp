...
OrderedCalls<CalleeType>::OrderedCalls(const ::std::vector<GenericCallee>& a_callees)
	:
	m_orderedCalls_p(new OrderedCalls<CalleeType>())
{
    const size_t calleesCount = a_callees.size();
    ...
    for(size_t i(0); i<calleesCount;++i){
        ...
        m_orderedCalls_p->m_callees[i]->m_callee = a_callees[i];
    }
}
