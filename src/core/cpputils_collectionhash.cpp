//
// repo:            cpputils
// file:			cpputils_collectionhash.cpp
// path:			src/core/cpputils_collectionhash.cpp
...
Hash::Hash(size_t a_numberOfBaskets, TypeCinternalAllocator a_allocator, TypeCinternalDeallocator a_deallocator)
    :
    m_clhash_data_p(CreateCollectionHash_p(a_allocator))
{
    new(m_clhash_data_p) Hash_p(a_numberOfBaskets,a_allocator,a_deallocator);
}
    ...
    m_dataFncs(CPPUTILS_NULL)
{
    ...
    m_dataFncs = (__private::SDataFunctions*)(*(m_hash->allocator))(sizeof(__private::SDataFunctions));
    if (!m_dataFncs) {
        throw ::std::bad_alloc();
    }
...
int32_t Hash_p::GetNextDataIndex(const __private::TypeCallDestructFnc& a_callDestructFnc)
{
    ...
    if(cunThisFuncIndex>=m_numberOfAllocatedTables){
        ...
        __private::SDataFunctions* const dataFncs = m_dataFncs;
        m_dataFncs = (__private::SDataFunctions*)(*(m_hash->allocator))(numberOfAllocatedTablesNew * sizeof(__private::SDataFunctions));
        if (!m_dataFncs) {
            throw ::std::bad_alloc();
        }
        memcpy(m_dataFncs,dataFncs,m_numberOfAllocatedTables * sizeof(__private::SDataFunctions));
...
//const __private::SDataFunctions& Hash_p::getDataFunction(int32_t a_index)const
//{
//    assert(a_index<m_counterOfDataTypes);
//    return m_dataFncs[a_index];
//}
//
//
//const __private::SKeyAndHasherFunctions& Hash_p::getKeyAndHasherFunction(int32_t a_index)const
//{
//    assert(a_index<m_counterOfKeyAndHashTypes);
//    return m_keyAndHasherFncs[a_index];
//}
